...
export class UserService {
  ...
  // 通过Username查找非github用户信息 user_info
  findDataByName(name) {
    const _sql = 'SELECT * FROM user_info WHERE name = ? and github_id IS NULL;';
    return query(_sql, name);
  }

  // 通过用户id查找用户信息 user_info 包括Username，性别，头像，最后Log in时间，状态等，不包括password
  getUserInfo(user_id) {
    const _sql =
  ...
  // 通过Username查找用户信息 user_info 不包括password
  ...
  // 通过用户id查找用户信息 user_info 包括password
